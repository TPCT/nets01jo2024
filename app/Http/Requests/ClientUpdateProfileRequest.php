...
class ClientUpdateProfileRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'phone'            => ['required' , 'max:25' , 'unique:clients,phone,' . auth()->id() ],
            'work_mobile'      => ['nullable' , 'max:25'],
            'home_mobile'      => ['nullable' , 'max:25'],
            'share_data'       => ['nullable' , 'max:25'],
            'email'            => ['nullable' ,'max:255' , 'email' , 'unique:clients,email,'  . auth()->id() ],
            ...
            'phones.*.country_code' => ['nullable' , 'max:5'],
            'phones.*.phone'   => ['nullable' , 'max:25'],
        ];
    }
